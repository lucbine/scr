...
type TB interface {
	...
	/*
		其中私有接口private()用于控制该接口的唯一性，即便用户代码中某个类型实现了这些方法，
...
type T struct {
	...
	context    *testContext // For running tests and subtests.// 控制测试的并发调度
}
...
//Fail()方法会标记当前测试为失败，然后继续运行，并不会立即退出当前测试。
//如果是子测试，则除了标记当前测试结果外还通过c.parent.Fail()来标记父测试失败。
func (c *common) Fail() {
	if c.parent != nil {
		c.parent.Fail()
	}
...
//可以简单的把一个测试理解为一个协程，FailNow()只会退出当前协程，并不会影响其他测试协程，
//但要保证在当前测试协程中调用FailNow()才有效，不可以在当前测试创建的协程中调用该方法。
...
//common.skip()方法标记当前测试为已跳过状态，比如测试中检测到某种条件，不再继续测试。
//该函数仅标记测试跳过，与测试结果无关。测试结果仍然取决于common.failed。
func (c *common) skip() {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.skipped = true
...
type testContext struct {
	...
	//等待并发执行的测试个数，所有等待执行的测试都阻
	numWaiting int

	// maxParallel is a copy of the parallel flag.
	//最大并发数，默认为系统CPU数，可以通过参数
	maxParallel int
}
